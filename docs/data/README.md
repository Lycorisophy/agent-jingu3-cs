...
| 路径 | 说明 |
|------|------|
| [migration/](migration/) | **Flyway 版本化 DDL**（`V1__*.sql` … `V7__*.sql`），为单一事实来源；构建时复制到 `agent-jingu3-cs-server` 的 classpath `db/migration` 由 Flyway 执行。 |
...
3. 在库内**按版本号顺序**执行 `migration/V1__*.sql` … `V8__*.sql`（勿只执行其中一段导致外键/依赖缺失）。**V5** 为记忆向量映射表 `memory_embedding`（与 Milvus 索引对账）；**V7** 为 `user_prompt_cipher`（可选用户提示词密文审计）；**V8** 为事实元数据时间维度与确认列。
