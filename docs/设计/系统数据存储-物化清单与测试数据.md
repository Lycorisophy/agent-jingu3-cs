...
| V8 | [V8__memory_fact_temporal.sql](../data/migration/V8__memory_fact_temporal.sql) | `fact_metadata` 增加 `temporal_tier`、`confirmed_at` 及历史 FACT 补行 |
| V9 | [V9__workspace_phase3.sql](../data/migration/V9__workspace_phase3.sql) | v0.7 Workspace Phase 3：`workspace`、`workspace_execution` |

构建时 `docs/data/migration/*.sql` 会复制到 `classpath:db/migration`（见 `agent-jingu3-cs-server/pom.xml` 中 `build.resources`；仓库根 [`pom.xml`](../../pom.xml) 为 Maven 父工程）。
...
| user_skill | 用户订阅技能 |
| memory_entry | 记忆条目 |
| fact_metadata | Fact 标签（挂 `memory_entry_id`）；`temporal_tier`（PERMANENT/LONG_TERM/SHORT_TERM）、`confirmed_at`（事实确认 MVP） |
...
| workspace | 工作空间元数据（每用户一行，`root_path` 与磁盘对齐；`quota_mb` 与配置默认一致） |
| workspace_execution | 沙箱执行历史摘要（stdout/stderr 截断；级联随 `workspace` 删除） |
