...
- **主索引 `events`**（字段与映射见第二版 **5.1**）：`event_id`, `session_id`/`conversation_id`, `user_id`, `timestamp`, `action`, `result`, `metadata`, **`vector_id`** 等。  
...
- 节点 **Event**：`id`, `type`, `timestamp`（轻量）。  
- 关系：**CAUSED**, **PRECEDES**, **REFERENCES** 等（第二版 **5.3**）。  
- 内容与全文仍回 **ES**，避免图库膨胀。
