...
public class ScheduledTaskService {
    ...
    public ScheduledTaskVo create(CreateScheduledTaskRequest request) {
        if (request.getScope() == ScheduledTaskScope.CONVERSATION
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "CONVERSATION 任务必须提供 conversationId");
        }
        if (request.getScope() == ScheduledTaskScope.GLOBAL && request.getConversationId() != null) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "GLOBAL 任务不应携带 conversationId");
        }
        String payloadJson;
        try {
    ...
    static String truncateError(String raw) {
        if (raw == null) {
            return null;
        }
    ...
    static String resolveConversationId(ScheduledTaskEntity task, String fromPayload) {
