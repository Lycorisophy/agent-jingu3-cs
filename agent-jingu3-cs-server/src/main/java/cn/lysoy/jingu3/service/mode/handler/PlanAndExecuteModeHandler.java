...
import cn.lysoy.jingu3.service.mode.graph.PlanExecuteGraphSupport;
 ...
 * <p>
 * 编排由 LangGraph4j 状态图实现（parse → exec_step 循环 → finalize）；子任务内工具路由仍经 {@link ToolStepService}。
 * 失败时可选<strong>单次</strong> Replanner（{@code jingu3.engine.plan.replan-enabled}）。
 * </p>
 */
@Slf4j
@Component
public class PlanAndExecuteModeHandler implements ActionModeHandler {
    ...
    /**
     * 流式：第 1 步为计划全文；后续子任务由 LangGraph4j 管线推送；若触发重规划则插入 {@code replan} 步再递归执行新计划。
     */
    ...
    private void streamWithPlan(
            ...
            int nextStepIndex) {
        AtomicInteger stepSlot = new AtomicInteger(nextStepIndex);
        try {
            PlanExecuteGraphSupport.invokeStream(
                    ctx, plan, originalUserMessage, toolStepService, maxSubtasks, sink, stepSlot);
            sink.done();
        } catch (RuntimeException ex) {
            log.warn("plan step failed: {}", ex.toString());
            if (replanEnabled && !afterReplan) {
                int replanAt = Math.max(nextStepIndex, stepSlot.get() - 1);
                sink.stepBegin(replanAt, "replan");
                ...
                sink.stepEnd(replanAt);
                streamWithPlan(ctx, newPlan, originalUserMessage, true, sink, stepSlot.get());
                return;
            }
    ...
    private String executeWithPlan(ExecutionContext ctx, String plan, String originalUserMessage, boolean afterReplan) {
        try {
            return PlanExecuteGraphSupport.invokeSync(ctx, plan, originalUserMessage, toolStepService, maxSubtasks);
        } catch (RuntimeException ex) {
            log.warn("plan step failed: {}", ex.toString());
            if (replanEnabled && !afterReplan) {
                String newPlan = chat.generate(
