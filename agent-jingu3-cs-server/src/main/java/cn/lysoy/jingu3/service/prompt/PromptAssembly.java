 ...
 * 与 {@link cn.lysoy.jingu3.skill.tool.ToolRoutingParser} 产出的 JSON 约定闭合。
 * </p>
 */
@Component
public class PromptAssembly {
    ...
    public String buildAskCombinedPrompt(ExecutionContext ctx) {
        return withModePreamble(
                ...
                PromptTemplates.ASK_SYSTEM
                        ...
                        + PromptTemplates.COGNITIVE_ALIGNMENT_TRUTHFULNESS
                        + PromptFragments.PARAGRAPH_BREAK
                        + recentDialogueBlock(ctx)
    ...
    /**
     * 指南 §5 第二阶段：在已定计划下执行子任务（由 {@link cn.lysoy.jingu3.service.mode.handler.PlanAndExecuteModeHandler} 循环调用）。
     */
    ...
    private static String recentDialogueBlock(ExecutionContext ctx) {
        List<String> h = ctx.getHistory();
        if (h == null || h.isEmpty()) {
            return "";
        }
        return "【近期对话】\n" + String.join("\n", h) + PromptFragments.PARAGRAPH_BREAK;
    }
}
