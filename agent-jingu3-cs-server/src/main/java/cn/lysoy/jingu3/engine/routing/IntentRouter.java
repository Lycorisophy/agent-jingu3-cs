 ...
 * 与「决策树」文档互补——此处为运行时实现。
 * <p>显式 mode 无法解析时降级为 {@link ActionMode#REACT}，来源 {@link RoutingSource#FALLBACK}。</p>
...
public class IntentRouter {
     ...
     * @return 永不为 {@code null}；非法显式 mode 时模式为 REACT、来源为 FALLBACK
     */
    public RoutingDecision resolve(String userMessage, String modeFromClient) {
        ...
        if (modeFromClient != null && !modeFromClient.isBlank()) {
            try {
                ...
                log.warn("invalid explicit mode [{}], fallback REACT", modeFromClient);
                return new RoutingDecision(ActionMode.REACT, RoutingSource.FALLBACK, "invalid_explicit_mode");
            }
        }
