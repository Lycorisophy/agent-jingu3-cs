...
public class ExecutionContext {
    ...
    /** 本步选用的行动模式（由 {@link cn.lysoy.jingu3.engine.routing.IntentRouter}、显式 mode、modePlan 或回落逻辑决定） */
    private final ActionMode selectedMode;
    /** 该模式来自客户端显式、关键词规则、模型分类、降级或显式守门等，供 {@link cn.lysoy.jingu3.prompt.ModeRoutingPreamble} 生成对模型的说明 */
    ...
    /** 选用 {@link ActionMode#WORKFLOW} 时必填的工作流定义 id；缺失时路由层可回落为 ASK，见 {@link cn.lysoy.jingu3.engine.routing.RoutingFallbacks} */
